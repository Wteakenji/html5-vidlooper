    ...
    <?php 
       ...
       if($number_of_rows > 0 && $rid != 'rec'){ //$rid != NULL
        ...
        if($rid != NULL){
          ...
          $vidrow = mysqli_fetch_array($mvid);
          header("Location: https://www.localese.me/video.php?vid=".$vid."&rid=".$vidrow{'record_id'});
        }
...
function uploadToPHPServer(blob) {
    ...
    makeXMLHttpRequest('https://www.localese.me/save.php', formData, function() {
        var downloadURL = 'https://www.localese.me/uploads/' + file.name;
  ...
  for (var i = 0; i < viditems.length; i++)
  {
    ...
    if(c[0].title == curid){
      if(limreach == 1){
        ...
        window.location = "https://www.localese.me/video.php?vid="+<?php echo $vid; ?>+"&rid=rec";
      }else{
        console.log('next page');
        var cnext = viditems[i+1].childNodes;
        window.location = "https://www.localese.me/video.php?vid="+<?php echo $vid; ?>+"&rid="+cnext[0].title;
      }
